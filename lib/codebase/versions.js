...
var storage = JMSstorage(config, log);
...
function add (sourceId, stage, originalName, hashedName) {
	...
	storage('getVersions', sourceId, stage, originalName, onHget.bind(null, sourceId, stage, originalName, hashedName));
}
...
function purgeSource (sourceId, stage, list, updatedList, done) {
	...
	keys.forEach(function (key) {
		storage('removeModules', sourceId, stage, key, function () {
			l--;
			if (l < 1) {
				storage('setVersions',sourceId, stage, updatedList, done);
			}
		});
	})
}
